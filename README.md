...
托盘菜单、现代设置窗口、快捷键／配色／高级字体对话框及“关于”页面支持简体中文和英文，默认跟随 Windows 用户区域语言。
...
可选值为 `auto`、`zh-CN` 和 `en-US`。语言从部署后的配置读取，修改后重新部署生效。
已打开的部署程序需重新打开才能使用新语言。旧版部署对话框和部分前端通知尚未迁移；方案名称、用户内容及 Rime 提供的状态标签保留原文。
