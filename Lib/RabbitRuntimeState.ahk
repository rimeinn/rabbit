...
UpdateStateLabels() {
    local str, slice
    global rime, session_id, ASCII_MODE_FALSE_LABEL, ASCII_MODE_TRUE_LABEL, ASCII_MODE_FALSE_LABEL_ABBR
    global ASCII_MODE_TRUE_LABEL_ABBR, FULL_SHAPE_FALSE_LABEL, FULL_SHAPE_TRUE_LABEL, FULL_SHAPE_FALSE_LABEL_ABBR
    global FULL_SHAPE_TRUE_LABEL_ABBR, ASCII_PUNCT_FALSE_LABEL, ASCII_PUNCT_TRUE_LABEL, ASCII_PUNCT_FALSE_LABEL_ABBR
    ...
    ASCII_MODE_FALSE_LABEL_ABBR := (slice && slice.slice !== "") ? slice.slice : "中"
    slice := rime.get_state_label_abbreviated(session_id, "ascii_mode", true, true)
    ASCII_MODE_TRUE_LABEL_ABBR := (slice && slice.slice !== "") ? slice.slice : "西"
    ...
    FULL_SHAPE_FALSE_LABEL_ABBR := (slice && slice.slice !== "") ? slice.slice : "半"
    slice := rime.get_state_label_abbreviated(session_id, "full_shape", true, true)
    FULL_SHAPE_TRUE_LABEL_ABBR := (slice && slice.slice !== "") ? slice.slice : "全"
    ...
    ASCII_PUNCT_FALSE_LABEL_ABBR := (slice && slice.slice !== "") ? slice.slice : "。"
    slice := rime.get_state_label_abbreviated(session_id, "ascii_punct", true, true)
    ASCII_PUNCT_TRUE_LABEL_ABBR := (slice && slice.slice !== "") ? slice.slice : "."
}
...
UpdateWinAscii(target := false, use_target := false, proc_name := "", by_tray_icon := false) {
    ...
    if use_target {
        ...
        ; Restore ascii_mode when returning to a previously active window.
        ...
        ; Default ascii_mode to false for processes not listed in the preset.
        target := false
        RabbitGlobals.process_ascii[proc_name] := !!target
        if current !== target {
            rime.set_option(session_id, "ascii_mode", target)
